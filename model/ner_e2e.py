...
class NEREnsemble(pl.LightningModule):
    def __init__(self,
                 train_data=None,
                 dev_data=None,
                 lr=1e-6,
                 ...
                 encoder_model2='xlm-roberta-base',
                 num_gpus=1):
        super(NEREnsemble, self).__init__()
        ...
        self.encoder_model2 = encoder_model2
        self.encoder2 = AutoModel.from_pretrained("xlm-roberta-base", return_dict=True)

        #self.encoder_model3 = encoder_model2
        #self.encoder3 = AutoModel.from_pretrained("xlm-roberta-large", return_dict=True)

        self.feedforward1 = nn.Linear(in_features=self.encoder.config.hidden_size, out_features=self.target_size)
        self.feedforward2 = nn.Linear(in_features=self.encoder2.config.hidden_size, out_features=self.target_size+13)
        #self.feedforward3 = nn.Linear(in_features=self.encoder3.config.hidden_size, out_features=self.target_size)

        self.feedforward_out = nn.Linear(in_features=2*self.target_size+13, out_features=self.target_size)
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(
            [
                ...
                {"params": self.encoder2.parameters(), "lr":self.lr},
                {"params": self.feedforward1.parameters(), "lr":self.lr_decoder},
                {"params": self.feedforward2.parameters(), "lr":self.lr_decoder},
                #{"params": self.feedforward3.parameters(), "lr":self.lr_decoder},
                {"params": self.feedforward_out.parameters(), "lr":self.lr_decoder},
                {"params": self.crf_layer.parameters(), "lr":self.lr_decoder},
    ...
    def perform_forward_step(self, batch, mode=''):
        ...
        embedded_text_input1 = self.encoder(input_ids=tokens, attention_mask=mask)
        embedded_text_input1 = embedded_text_input1.last_hidden_state
        embedded_text_input1 = self.dropout(F.leaky_relu(embedded_text_input1))
        token_scores1 = self.feedforward1(embedded_text_input1)
        token_scores1 = F.log_softmax(token_scores1, dim=2)

        embedded_text_input2 = self.encoder2(input_ids=tokens, attention_mask=mask)
        embedded_text_input2 = embedded_text_input2.last_hidden_state
        embedded_text_input2 = self.dropout(F.leaky_relu(embedded_text_input2))
        token_scores2 = self.feedforward2(embedded_text_input2)
        token_scores2 = F.log_softmax(token_scores2, dim=2)

        #embedded_text_input3 = self.encoder3(input_ids=tokens, attention_mask=mask)
        #embedded_text_input3 = embedded_text_input3.last_hidden_state
        #embedded_text_input3 = self.dropout(F.leaky_relu(embedded_text_input3))
        #token_scores3 = self.feedforward3(embedded_text_input3)

        token_scores = torch.cat((token_scores1, token_scores2), dim=2)
        #token_scores = torch.sum(torch.stack([token_scores1, token_scores2]), dim=0)
        #token_scores = torch.cat((token_scores, token_scores3), dim=2)
        
        token_scores = self.feedforward_out(token_scores)
        token_scores = F.log_softmax(token_scores, dim=-1)
        if mode=="ensemble_sm":
            return F.softmax(token_scores, dim=2)
        # compute the log-likelihood loss and compute the best NER annotation sequence
        output = self._compute_token_tags(token_scores=token_scores,mask=mask, tags=tags, token_mask=token_mask, metadata=metadata, batch_size=batch_size, mode=mode)
        

        return output

    def _compute_token_tags(self, token_scores, mask, tags, token_mask, metadata, batch_size, mode=''):
        # compute the log-likelihood loss and compute the best NER annotation sequence
        ...
        #loss = loss(torch.permute(F.log_softmax(token_scores, dim=2), dims=(0,2,1)), tags)
        #best_path = torch.argmax(token_scores, dim=2)
