...
from utils.utils import parse_args, get_reader, load_ensemble_model, get_out_filename, get_tagset

if __name__ == '__main__':
    os.environ['TRANSFORMERS_CACHE'] = "/scratch/project_2003550/tmp/multiconer/models"
    timestamp = time.time()
    sg = parse_args()
    ...
    model = load_ensemble_model(sg.model, tag_to_id=get_tagset(sg.iob_tagging))
    model = model.to(sg.cuda)
    # use pytorch lightnings saver here.
    eval_file = get_out_filename(sg.out_dir, sg.file_name, prefix=sg.prefix)
