...
from model.ner_e2e import NEREnsemble
...
def parse_args():
    ...
    p.add_argument('--emea', type=str, help="Do emea if true", default="")
    p.add_argument('--file_name', type=str, help="File name to save", default="from_batch")
    return p.parse_args()
...
def create_ensemble_model(train_data, dev_data, tag_to_id, batch_size=64, dropout_rate=0.1, stage='fit', lr=1e-5, encoder_model='xlm-roberta-base', num_gpus=1):
    return NEREnsemble(train_data=train_data, dev_data=dev_data, tag_to_id=tag_to_id, batch_size=batch_size, stage=stage, encoder_model=encoder_model, encoder_model2=encoder_model,
                            dropout_rate=dropout_rate, lr=lr, pad_token_id=train_data.pad_token_id, num_gpus=num_gpus)
...
def load_model(model_file, tag_to_id=None, stage='test'):
    ...
    model = NERBaseAnnotator.load_from_checkpoint(model_file,stage=stage, tag_to_id=tag_to_id)#, hparams_file=hparams_file, stage=stage, tag_to_id=tag_to_id)
    model.stage = stage
    return model


def load_ensemble_model(model_file, tag_to_id=None, stage='test'):
    #hparams_file = model_file[:model_file.rindex('checkpoints/')] + '/hparams.yaml'
    model = NEREnsemble.load_from_checkpoint(model_file,stage=stage, tag_to_id=tag_to_id)#, hparams_file=hparams_file, stage=stage, tag_to_id=tag_to_id)
    model.stage = stage
    return model
...
def train_model(model, out_dir='', epochs=10, gpus=1, seed=42):
    trainer = get_trainer(gpus=gpus, out_dir=out_dir, epochs=epochs, seed=seed)
    trainer.fit(model)
    return trainer


def get_trainer(gpus=4, is_test=False, out_dir=None, epochs=10, seed=42):
    seed_everything(seed)
    checkpoint_callback = ModelCheckpoint(dirpath='/scratch/project_2003550/tmp/multiconer/models/ADD_HERE/self_ensemble/', save_top_k = 0, save_weights_only=TRUE)
    if is_test:
        return pl.Trainer(gpus=1) if torch.cuda.is_available() else pl.Trainer(val_check_interval=100)

    if torch.cuda.is_available():
        trainer = pl.Trainer(gpus=gpus, deterministic=False, max_epochs=epochs, callbacks=[get_model_earlystopping_callback(), checkpoint_callback],
                             default_root_dir=out_dir, accelerator="gpu", strategy="ddp")
